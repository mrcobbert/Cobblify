...
- Version bump for the release: every source `tools/check-release-version.sh`
  ...
  plus the `cobblify-launcher` entry in `Cargo.lock`. 0.15.0 as of 2026-09-20
  ...
  Candidate workflow, see `UPDATE_RELEASE.md` "Dev channel"; 0.15.0: one
  global `/bw mode` for every stats surface, labelled when forced, atomic
  settings save, Worker per-mode parse guard - mod-side only, no launcher
  code change).
