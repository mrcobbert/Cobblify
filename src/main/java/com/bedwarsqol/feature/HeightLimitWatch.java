...
/**
 * Forge adapter for {@link HeightLimitCore}. A netty handler ahead of the vanilla packet handler
 * (the pattern the official Forge Mod API mod uses) sees every custom payload and every block
 * placement request: {@code hypixel:hello} triggers our {@code hypixel:register}, {@code
 * hyevent:location} names the map, and each C08 becomes a placement attempt the core settles a
 * moment later against the world. Chat supplies Hypixel's "Build height limit reached!" verdict and
 * the tick drives world-change expiry and the sidebar {@code Map:} fallback. Everything reaches the
 * core on the client thread. The register message is only ever sent after Hypixel's own hello and
 * only on the exact Hypixel host; off Hypixel this class sends nothing.
 */
public final class HeightLimitWatch {

    private static final String HANDLER_NAME = "cobblify_height_limit";
    ...
    public void onServerConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        if (event == null || event.manager == null) return;
        try {
            if (event.manager.channel().pipeline().get(HANDLER_NAME) != null) return;
            event.manager.channel().pipeline().addBefore("packet_handler", HANDLER_NAME, new PacketTap());
        } catch (RuntimeException e) {
            DiagLog.log("heightlimit: pipeline hook failed " + e);
        }
    }
    ...
    /** Reads (never consumes) the payloads and placements passing through the connection. */
    private static final class PacketTap extends ChannelDuplexHandler {
        @Override
