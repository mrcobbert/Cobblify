 ...
 *
 * <p>Provenance: {@link #maxY} is Lunar Client's own Hypixel Bedwars table ({@code hypixel/bedwars.json},
 * {@code build_heights}, fetched from Lunar's launch manifest 2026-09-14; 201 maps). Lunar stores the
 * first <i>denied</i> Y and displays one less (Lighthouse: 111 stored, 110 on the HUD), so every value
 * here is Lunar's minus one - the number its HUD shows. Two typo duplicates ({@code build_side},
 * {@code pharoah}) are dropped and {@code mortuss} is spelled Mortuus. Floors and radii (Lunar has
 * neither) and the team pool come from the community HeightLimitMod / Bed Wars Map List datasets,
 * which also supply Haven and Turtle Cove, absent from Lunar. Of the 177 maps in both, 159 agreed with
 * the community data after the shift; the rest follow Lunar.
 *
...
public final class HeightLimitTable {
    ...
    static {
        HeightLimitMap[] all = {
            ...
            m("Echo Ruins", 8, 68, 98, 0),
            m("Egg Factory", 0, 0, 92, 0),
            ...
            m("Temple", 4, 43, 106, 100),
            m("Tengshe", 4, 64, 100, 114),
            m("Terminal", 4, 58, 87, 93),
            m("Terraced", 8, 0, 89, 0),
            m("Tigris", 4, 68, 106, 115),
            m("Tinselbury", 0, 0, 100, 0),
            m("Toro", 8, 65, 92, 117),
            m("Treenan", 4, 47, 121, 100),
